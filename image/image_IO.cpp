...
#include "png.h"
    namespace libtiff {
        #include "tiffio.h" // That will solve compilation issue although I am not sure if this solution is
        //convenient for use, as you will have to use libtiff members using libtif:: prefix,
    }
#include "jpeglib.h"
}
...
namespace image {
    ...
    int ReadTiff(const char * filename,
                 ...
                 int * depth)
    {
        libtiff::TIFF* tiff = libtiff::TIFFOpen(filename, "r");
        if (!tiff) {
            std::cerr << "Error: Couldn't open " << filename << " fopen returned 0";
            return 0;
        }
        libtiff::uint16 bps, spp;
        ...
        *depth = bps * spp / 8;
        ...
        if (*depth==4) {
            if (ptr != NULL) {
                if (!TIFFReadRGBAImageOriented(tiff, *w, *h, (libtiff::uint32*)&((*ptr)[0]), ORIENTATION_TOPLEFT, 0)) {
                    TIFFClose(tiff);
                    return 0;
                }
            }
            ...
            for (size_t i=0; i<TIFFNumberOfStrips(tiff); ++i) {
                if (TIFFReadEncodedStrip(tiff, i, ((libtiff::uint8*)&((*ptr)[0]))+i*TIFFStripSize(tiff),(libtiff::tsize_t)-1) ==
                    std::numeric_limits<libtiff::tsize_t>::max()) {
                    TIFFClose(tiff);
                    return 0;
                }
            }
    ...
    int WriteTiff(const char * filename,
                  ...
                  int depth)
    {
        libtiff::TIFF* tiff = libtiff::TIFFOpen(filename, "w");
        if (!tiff) {
            std::cerr << "Error: Couldn't open " << filename << " fopen returned 0";
            return 0;
        }
        ...
        for (libtiff::uint32 y=0; y < h ; ++y) {
            if (TIFFWriteScanline(tiff,(libtiff::tdata_t)(((libtiff::uint8*)(&ptr[0]))+depth*w*y),y)<0) {
                TIFFClose(tiff);
                return 0;
            }
        }
